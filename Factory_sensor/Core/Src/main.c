...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  HAL_TIM_Base_Start(&htim1);  // 타이머 한 번만 시작
  DHT11_Init();
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  HAL_UART_Transmit(&huart2, (uint8_t*)"[READ START]\r\n", 14, 100);
	  DHT11_Data dht = DHT11_Read();
	  HAL_UART_Transmit(&huart2, (uint8_t*)"[READ END]\r\n", 12, 100);

	  char tx_buf[32];
	  if (dht.valid) {
		  snprintf(tx_buf, sizeof(tx_buf), "%d,%d\r\n", dht.temperature, dht.humidity);
	  } else {
		  snprintf(tx_buf, sizeof(tx_buf), "ERR\r\n");
	  }
